"""
Script para insertar datos de tabla_2017.csv en la tabla Estudiantes_2017
...
ruta_archivo = os.path.join(project_root, "CSVs", "tabla_2017.csv")

print("\n" + "="*70)
print("INSERCIÓN DE DATOS - TABLA ESTUDIANTES_2017")
print("="*70)
...
try:
    ...
    with engine.connect() as connection:
        # Total de registros
        total_query = connection.execute(text("SELECT COUNT(*) FROM Estudiantes_2017"))
        total = total_query.scalar()
        
        # Distribución por año
        print(f"\n📈 Distribución de datos en Estudiantes_2017:")
        print(f"   • Total de registros: {total}")
        
        # Verificar año
        año_query = connection.execute(text(
            "SELECT DISTINCT FECHA, COUNT(*) as cantidad FROM Estudiantes_2017 GROUP BY FECHA"
        ))
        ...
        pob_query = connection.execute(text(
            "SELECT POBLACION, COUNT(*) as cantidad FROM Estudiantes_2017 GROUP BY POBLACION ORDER BY cantidad DESC LIMIT 5"
        ))
        print(f"\n   Población (top 5):")
        ...
        nivel_query = connection.execute(text(
            "SELECT NIVEL, COUNT(*) as cantidad FROM Estudiantes_2017 GROUP BY NIVEL ORDER BY NIVEL"
        ))
        print(f"\n   Niveles:")
        ...
        dia_query = connection.execute(text(
            "SELECT DIA, COUNT(*) as cantidad FROM Estudiantes_2017 GROUP BY DIA ORDER BY cantidad DESC"
        ))
        print(f"\n   Días:")
    ...
    logger.info(f"Successfully inserted {inseridos} records into Estudiantes_2017")
