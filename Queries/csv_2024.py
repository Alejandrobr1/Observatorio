...
ruta_archivo = os.path.join(project_root, "CSVs", "data_2024.csv")
...
TIPO_DOCUMENTOS_2024 = df[["TIPO DE IDENTIFICACIÓN"]].drop_duplicates().reset_index(drop=True)
TIPO_DOCUMENTOS_2024 = TIPO_DOCUMENTOS_2024[TIPO_DOCUMENTOS_2024["TIPO DE IDENTIFICACIÓN"] != 'SIN INFORMACION']

NIVEL_MCER_2024 = df[["NIVEL_MCER","TIPO POBLACION","ESTADO ETAPA 2","ANIO","IDIOMA","CERTIFICADO","GRADO"]].drop_duplicates().reset_index(drop=True)

INSTITUCIONES_2024 = df[["INSTITUCIÓN EDUCATIVA","COLEGIO ABREVIADO PARA LISTADOS"]].drop_duplicates().reset_index(drop=True)
INSTITUCIONES_2024 = INSTITUCIONES_2024[INSTITUCIONES_2024["INSTITUCIÓN EDUCATIVA"] != 'SIN INFORMACION']

CIUDADES_2024 = df[["MUNICIPIO"]].drop_duplicates().reset_index(drop=True)
CIUDADES_2024 = CIUDADES_2024[CIUDADES_2024["MUNICIPIO"] != 'SIN INFORMACION']

CURSOS_2024 = df[["ENTIDAD","IDIOMA","INSTITUCIÓN EDUCATIVA","NOMBRE_CURSO_PROCESADO","TIPO POBLACION"]].copy()
...
PERSONA_NIVEL_2024 = df[["NÚMERO DE IDENTIFICACIÓN","NIVEL_MCER","TIPO POBLACION","ANIO"]].copy()

# DEBUG: Información de NIVEL_MCER
print(f"\n🔍 DEBUG - Datos de NIVEL_MCER en CSV:")
print(f"  - Total filas: {len(PERSONA_NIVEL_2024)}")
print(f"  - Con NIVEL_MCER != 'SIN INFORMACION': {len(PERSONA_NIVEL_2024[PERSONA_NIVEL_2024['NIVEL_MCER'] != 'SIN INFORMACION'])}")
print(f"  - Niveles únicos: {sorted(PERSONA_NIVEL_2024['NIVEL_MCER'].unique())}")
print(f"  - Años únicos: {sorted(PERSONA_NIVEL_2024['ANIO'].unique())}")

SEDES_2024 = df[["GRUPO","JORNADA","SEDE NODAL","NÚMERO DE IDENTIFICACIÓN"]].copy()

PERSONAS_2024['NÚMERO DE IDENTIFICACIÓN'] = PERSONAS_2024['NÚMERO DE IDENTIFICACIÓN'].apply(
    lambda x: 'Sin información' if pd.isna(x) or str(x).strip().lower() in ['nan', 'none', '', 'sin informacion'] else str(x).strip()
)

PERSONA_NIVEL_2024['NÚMERO DE IDENTIFICACIÓN'] = PERSONA_NIVEL_2024['NÚMERO DE IDENTIFICACIÓN'].apply(
    lambda x: 'Sin información' if pd.isna(x) or str(x).strip().lower() in ['nan', 'none', '', 'sin informacion'] else str(x).strip()
)

SEDES_2024['NÚMERO DE IDENTIFICACIÓN'] = SEDES_2024['NÚMERO DE IDENTIFICACIÓN'].apply(
    lambda x: 'Sin información' if pd.isna(x) or str(x).strip().lower() in ['nan', 'none', '', 'sin informacion'] else str(x).strip()
)

print("\nIniciando inserción de datos...")
print(f"Total personas a procesar: {len(PERSONAS_2024)}")
...
with engine.connect() as connection:
    ...
    for _, row in TIPO_DOCUMENTOS_2024.iterrows():
    ...
    for _, row in CIUDADES_2024.iterrows():
    ...
    for _, row in INSTITUCIONES_2024.iterrows():
    ...
    for _, row in NIVEL_MCER_2024.iterrows():
    ...
    for idx, row in PERSONAS_2024.iterrows():
    ...
    for _, row in PERSONA_NIVEL_2024.iterrows():
        numero_doc = row['NÚMERO DE IDENTIFICACIÓN']
    ...
    for _, row in SEDES_2024.iterrows():
        numero_doc = row['NÚMERO DE IDENTIFICACIÓN']
    ...
    for _, row in CURSOS_2024.iterrows():
