...
ruta_archivo = os.path.join(project_root, "CSVs", "data_2017.csv")
...
if grado_col != 'GRADO':
    NIVEL_MCER_2017 = NIVEL_MCER_2017.rename(columns={grado_col: 'GRADO'})

NIVEL_MCER_2017 = NIVEL_MCER_2017.drop_duplicates().reset_index(drop=True)

print(f"\n✓ NIVEL_MCER_2017 preparado: {len(NIVEL_MCER_2017)} registros únicos")
print(f"  Grados únicos: {sorted(NIVEL_MCER_2017['GRADO'].unique())}")
print(f"  Distribución:")
print(NIVEL_MCER_2017['GRADO'].value_counts())
...
CIUDADES_2017 = df[["MUNICIPIO"]].drop_duplicates().reset_index(drop=True)
CIUDADES_2017 = CIUDADES_2017[CIUDADES_2017["MUNICIPIO"] != 'SIN INFORMACION']

CURSOS_2017 = df[["ENTIDAD","IDIOMA","INSTITUCIÓN EDUCATIVA","NOMBRE_CURSO_PROCESADO","TIPO POBLACION"]].copy()
CURSOS_2017 = CURSOS_2017[CURSOS_2017["INSTITUCIÓN EDUCATIVA"] != 'SIN INFORMACION'].drop_duplicates().reset_index(drop=True)

PERSONAS_2017 = df[["NOMBRES","APELLIDOS","TELÉFONO 1","TELÉFONO 2","NÚMERO DE IDENTIFICACIÓN","CORREO ELECTRÓNICO",
...
PERSONA_NIVEL_2017 = df[["NÚMERO DE IDENTIFICACIÓN","NIVEL_MCER","TIPO POBLACION","ANIO","GRADO"]].copy()

SEDES_2017 = df[["GRUPO","JORNADA","SEDE NODAL","NÚMERO DE IDENTIFICACIÓN"]].copy()

PERSONAS_2017['NÚMERO DE IDENTIFICACIÓN'] = PERSONAS_2017['NÚMERO DE IDENTIFICACIÓN'].apply(
    lambda x: 'Sin información' if pd.isna(x) or str(x).strip().lower() in ['nan', 'none', '', 'sin informacion'] else str(x).strip()
)

PERSONA_NIVEL_2017['NÚMERO DE IDENTIFICACIÓN'] = PERSONA_NIVEL_2017['NÚMERO DE IDENTIFICACIÓN'].apply(
    lambda x: 'Sin información' if pd.isna(x) or str(x).strip().lower() in ['nan', 'none', '', 'sin informacion'] else str(x).strip()
)

SEDES_2017['NÚMERO DE IDENTIFICACIÓN'] = SEDES_2017['NÚMERO DE IDENTIFICACIÓN'].apply(
    lambda x: 'Sin información' if pd.isna(x) or str(x).strip().lower() in ['nan', 'none', '', 'sin informacion'] else str(x).strip()
)
...
with engine.connect() as connection:
    ...
    for _, row in TIPO_DOCUMENTOS_2017.iterrows():
    ...
    for _, row in CIUDADES_2017.iterrows():
    ...
    for _, row in INSTITUCIONES_2017.iterrows():
    ...
    for idx, row in PERSONAS_2017.iterrows():
    ...
    print("\n6. Procesando Sedes...")
    sedes_nuevas = 0
    for _, row in SEDES_2017.iterrows():
        numero_doc = row['NÚMERO DE IDENTIFICACIÓN']
        persona_id = connection.execute(text("SELECT ID FROM Personas WHERE NUMERO_DOCUMENTO = :numero_doc"), 
                                       {'numero_doc': numero_doc}).fetchone()
        if persona_id is None:
    ...
    print("\n7. Procesando Cursos...")
    cursos_nuevos = 0
    for _, row in CURSOS_2017.iterrows():
