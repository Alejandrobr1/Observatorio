...
def load_data(_engine, year):
    ...
    with _engine.connect() as connection:
        ...
        query = text(f"""
            SELECT 
                GRADO, NIVEL_MCER, COUNT(ID) as cantidad
            FROM {table_name}
            WHERE GRADO IS NOT NULL AND GRADO != '' AND GRADO != 'SIN INFORMACION'
              AND NIVEL_MCER IS NOT NULL AND NIVEL_MCER != '' AND NIVEL_MCER != 'SIN INFORMACION'
              AND FECHA = :year
            GROUP BY GRADO, NIVEL_MCER
            ORDER BY GRADO, NIVEL_MCER
        """)
        result = connection.execute(query, params)
        df = pd.DataFrame(result.fetchall(), columns=["GRADO", "NIVEL_MCER", "cantidad"])
        
        # Métricas
        total_matriculados = connection.execute(text(f"SELECT COUNT(ID) FROM {table_name} WHERE FECHA = :year"), params).scalar() or 0
