...
@st.cache_data
def load_data_by_language(_engine, year, population):
    table_name = "Estudiantes_intensificacion"
    with _engine.connect() as connection:
        if not _engine.dialect.has_table(connection, table_name):
...
try:
    ...
    df_idiomas, total_estudiantes, total_idiomas = load_data_by_language(engine, selected_year, st.session_state.population_filter)
