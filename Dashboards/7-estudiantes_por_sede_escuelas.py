...
with col_selector2:
    if dashboard_choice == "Estudiantes":
        report_choice = st.selectbox(
            "Seleccionar Reporte",
            ["Estudiantes por Grupo y Sede", "Otro Reporte de Estudiantes"],
...
def create_bar_chart_and_table(df_data, total_grupo, title):
    ...
    df_data = df_data[df_data['cantidad'] > 0] # Filtrar sedes con 0 estudiantes
...
try:
    with engine.connect() as connection:
        ...
        df_grupo1 = pd.DataFrame(result_grupo1.fetchall(), columns=["sede", "cantidad"])
        ...
        df_grupo2 = pd.DataFrame(result_grupo2.fetchall(), columns=["sede", "cantidad"])
        ...
        df_grupo3 = pd.DataFrame(result_grupo3.fetchall(), columns=["sede", "cantidad"])
