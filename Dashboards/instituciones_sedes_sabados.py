...
st.set_page_config(layout="wide", page_title="Dashboard Instituciones y Sedes Nodales - Formación Sábados")
st.title("🏫 Distribución de Estudiantes por Institución y Sede Nodal - FORMACIÓN SÁBADOS")
...
try:
    with engine.connect() as connection:
        # Consulta para obtener estudiantes por institución y sede nodal - SOLO FORMACION SABADOS Y ESTUDIANTES
        query = text("""
            SELECT 
        ...
        st.header(f"📊 Estudiantes por Institución y Sede Nodal - Formación Sábados - Año {selected_year}")
        ...
        ax.set_title(f'Distribución de Estudiantes por Institución y Sede Nodal - Formación Sábados\nAño {selected_year}',
        ...
        with col2:
            ...
            if not df_institucion.empty:
                ...
                titulo_pie = f'Distribución por Sede Nodal\n{institucion_seleccionada[:40]}...' if len(institucion_seleccionada) > 40 else f'Distribución por Sede Nodal\n{institucion_seleccionada}'
                ax_pie.set_title(titulo_pie, fontsize=11, fontweight='bold', pad=15)
        ...
        with st.expander("🔍 Ver tabla completa de todas las instituciones"):
            ...
            tabla_completa = []
            for institucion in instituciones_ordenadas[::-1]:
