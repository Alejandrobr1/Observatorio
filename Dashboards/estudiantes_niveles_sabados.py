...
st.set_page_config(layout="wide", page_title="Dashboard MCER por Genero - Formación Sábados")
st.title("📊 Estudiantes por Nivel MCER y Genero - FORMACIÓN SÁBADOS")
...
with engine.connect() as connection:
    # Obtener años disponibles con datos de FORMACION SABADOS (2016-2025)
    query_years = text("""
...
with engine.connect() as connection:
    ...
    # Personas estudiantes con FORMACION SABADOS
    ...
    st.sidebar.metric("Estudiantes Formación Sábados", f"{total_con_formacion:,}")

st.sidebar.divider()

# Consulta principal CON filtros de FORMACION SABADOS e INSTITUCION
try:
    with engine.connect() as connection:
        ...
        # Consulta principal - SOLO FORMACION SABADOS Y ESTUDIANTES
        query = text(f"""
            SELECT 
                n.NIVEL_MCER,
                p.GENERO,
        ...
        titulo_filtros = f"Formación Sábados - Estudiantes - {selected_year}"
        ...
        titulo_grafico = 'Distribución por Nivel MCER y Genero - Formación Sábados'
