...
st.set_page_config(layout="wide", page_title="Dashboard Docentes por Nivel")
...
def create_donut_chart_and_table(df_data, total_docentes, title):
    ...
    with col1:
        ...
        st.dataframe(df_display, width=True, hide_index=True)
        ...
        with st.expander("📅 **Seleccionar Año para Visualizar**", expanded=True):
            ...
            for i, year in enumerate(available_years):
                with cols[i]:
                    ...
                    if st.button(str(year), key=f"year_{year}", width=True, type=button_type, on_click=set_year, args=(year,)):
                        pass
